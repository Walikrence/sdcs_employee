...
public class EnterpriseController {
    ...
    public Result add( Enterprise enterprise) {
        ...
        if (null != enterprise.getId()) {
            //id是否已存在
            Condition condition = new Condition(Enterprise.class);
            Example.Criteria criteria = condition.createCriteria();
            criteria.andEqualTo("id", enterprise.getId());
            List<Enterprise> list = enterpriseService.findByCondition(condition);
            if (!list.isEmpty()) {
    ...
    @PostMapping("/addMany")
    public Result addMany(List<Enterprise> enterprises) {
        if (enterpriseService.save(enterprises)>0) {
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult("失败");
        }
    ...
    public Result deleteByIds(String ids) {
        ...
        if (enterpriseService.deleteByIds(ids)>0) {
            return ResultGenerator.genSuccessResult();
        }else {
    ...
    @GetMapping("/list")
    public Result<PageInfo<Enterprise>> list(@RequestParam(defaultValue = "0") Integer page,
